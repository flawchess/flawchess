...
- [ ] **INGEST-06**: Centipawn convention (signed from white's POV, centipawns vs pawn-units) verified against a known sample before scaling; verification documented in the ingestion script or a one-off validation note. (Original scope also added `eval_depth` and `eval_source_version` columns to `games`; both dropped on 2026-04-26 after the smoke ingest revealed Lichess's `/api/games/user` endpoint emits bare `[%eval cp]` annotations with no depth field, leaving both columns dead. Position-level eval presence is filterable via `game_positions.eval_cp IS NOT NULL`. If a future eval source needs a discriminator, reintroduce a column at that point.)
