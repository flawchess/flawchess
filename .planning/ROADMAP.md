...
- [ ] Phase 88.3: Endgame Stats viz refinements — timeline inactivity-gap annotations (axis-break glyph + "≈Ny inactive", mobile-sized); ELO Timeline defaults to single most-active series (toggleable, no tabs); "Endgame Overall Performance" arrow-flow replaced by one responsive 2-column card (Games without/with Endgame | Eval at Entry/Score Differences), equal height by construction. Frontend-only, no stats/benchmark change (TBD plans; INSERTED 2026-05-18) — planned
