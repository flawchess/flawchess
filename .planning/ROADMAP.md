...
- [ ] **Phase 94: Backend & Frontend Percentile Annotations** — Nullable `{metric}_percentile` on endgame responses + "top X% / bottom Y%" chip on 4 ΔES rows (desktop + mobile parity, metric-aware popover copy)
...
**Success Criteria** (what must be TRUE):
  ...
  2. The breakpoint tables are committed at `app/services/global_percentile_cdf.py` as a typed `GLOBAL_PERCENTILE_CDF` registry; the TS codegen step regenerates the frontend mirror cleanly and CI drift-guard passes on the Python→TS step.
