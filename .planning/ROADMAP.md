...
- ✅ **v2.17 Maia Latency & iOS Return** — Phase 219 plus the SEED-158 iOS return (shipped 2026-09-07; deployed to production throughout, releases #342–#355 incl. hotfixes #348/#349) — the analysis board's Human Move Probability chart appears roughly 7x faster on devices without WebGPU (onnxruntime-web re-pinned to 1.27.0 with a headless benchmark gate, cross-origin isolation shipped site-wide so Maia runs on up to four wasm threads, an 11-rung coarse paint refined in place; all three D-15 targets met: exact rung 81 ms, ladder 0.94 s, first paint 639 ms), then the iOS fallout was bisected on-device over two days and Maia came back on iPhone and iPad on the CPU wasm backend, with iOS move sounds moved to Web Audio; the range also carries the rated-human analytics default (SEED-163), versioned engine runtime URLs, persona card glow and the Generate Insights 400 fix — see [milestones/v2.17-ROADMAP.md](milestones/v2.17-ROADMAP.md)
- ✅ **v2.18 Flaw Data Integrity — Opening Eval Cache Repair & Tactic-Tagger Precision** — Phases 220–221 (shipped 2026-09-13; deployed to production throughout, releases #356–#358) — the poisoned first-write-wins opening-eval dedup cache was repaired end to end on prod (2,215,868 rows audited, 4,097 repaired, 7,191 spurious opening blunders removed across 4,079 games) and replaced by two-source confirmation with provenance, the remote-worker submit path now writing the cache (SEED-164, Phase 220); then every tactic tag got a solver-winning floor at the firing node, seven cook-port divergences were fixed, the clearance motif retired and all 4.76M prod flaws retagged, losing-line share 10.7% -> 0.01% (SEED-165, Phase 221); the range also carries the tilt data story — see [milestones/v2.18-ROADMAP.md](milestones/v2.18-ROADMAP.md)
- ✅ **v2.19 Train Bot-Narrated Onboarding & Verdicts** — Phase 222 (shipped 2026-09-14; deployed to production via release #359) — the Bots personas become the permanent voice of Train: guess prompt and buttons inside a bot bubble under the board with a first-session Tank/Hilda intro and a decide-first nudge, outcome-matched bot verdicts with inline point pills and the spaced-repetition return date, a first-reveal walkthrough, a bot-led score screen that explains spaced repetition before the reminder ask (desktop asks for the phone install first), server-side onboarding-seen state, the Train funnel on `/activity`, and a reworked-Train nav dot for every account (SEED-166) — see [milestones/v2.19-ROADMAP.md](milestones/v2.19-ROADMAP.md)
...
| 219. Maia Chart Latency — ORT 1.27 Re-pin, Cross-Origin Isolation & Progressive Ladder Paint (v2.17) | 3/3 | Complete    | 2026-09-06 |
