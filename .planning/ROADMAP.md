...
**Success Criteria** (what must be TRUE):
  ...
  5. **Reactive / user-initiated unlock — no forced reload.** No `window.location.reload()` remains on the eval-completion path; the `useEvalCoverage` auto-reload is retired. The import page's Tier-1 state offers an "Explore openings" CTA; Tier-2 reveals (Endgames unlock + Openings eval metrics appearing) happen reactively via the readiness poll + query invalidation.
