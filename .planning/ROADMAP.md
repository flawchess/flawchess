...
**Open planning inputs** (resolve during plan-phase, not blockers):
  ...
  - **Score Gap placement.** Since Score Gap is TC-flat, decide whether it sits per-tile inside every TC card (same band repeated, accepted because collapsed by default) or is hoisted once to a TC-agnostic position. Default: keep per-tile with the single per-class band.
  - **Accordion state.** Which TC counts as "most-active" (most eligible endgame games? most total games?) and whether expand/collapse state persists across filter changes.
...
**Success Criteria** (what must be TRUE):
  ...
  2. The user's **most-active TC card is expanded by default**; the others are collapsed behind a chevron and expand on click. Expand/collapse is keyboard-accessible with `data-testid`s on each header.
  ...
  5. **Score Gap retains a single per-class (TC-invariant) band** wherever it appears; it is not split into per-TC bands.
