...
**Goal**: Bring the entire flaw-tag stack (backend literals/constants/columns, DB, API, frontend) into line with the finalized taxonomy in [`.planning/notes/flaw-tag-definitions.md`](notes/flaw-tag-definitions.md) — rename the tempo residual pair, replace the outcome-dependent impact family with the outcome-independent `reversed` / `squandered` ladder, surface canonical tag names with hover/tap definition popovers, drop the chip→Flaws deep-links, and emphasize chips whose tag matches an active cross-tab Flaw filter.
...
**Scope** (the six asks):
  ...
  2. **Rename / rebuild, all the way through** —
     ...
     - *Impact*: **drop `while-ahead` entirely** (a state tag, not a swing); **replace `result-changing`** with the two-rung outcome-independent ladder `reversed` (ES ≥70% → ≤30%) and `squandered` (ES ≥85% → ≤60%, when not `reversed`), most-severe wins, at most one impact tag. This is a **behavioral** classifier change, not just a rename.
     ...
     - *Types / schemas*: `FlawTag` / `TempoTag` Literals (backend `flaws_service.py` + `frontend/src/types/library.ts`); `TagDistribution` rate fields (drop `while_ahead_rate` / `result_changing_rate`; add `reversed_rate` / `squandered_rate` if the panel surfaces impact rates).
     - *DB columns* (`game_flaws`): drop `is_while_ahead`, drop `is_result_changing`, add `is_reversed`, add `is_squandered`, via a **new Alembic alter migration** (do NOT edit the 20260606 create migration — that would force a dev-DB reset, which is disallowed). `is_miss` / `is_lucky_escape` unchanged.
  ...
  4. **Canonical names in the panel** — render tags as the literal `lowercase-with-dash` names (`low-clock`, `lucky-escape`, `reversed`, …) on card chips and in the Flaw-Stats panel, replacing the title-cased `TAG_LABELS`.
...
**Success Criteria** (what must be TRUE):
  ...
  2. The impact classifier emits at most one of `reversed` / `squandered` per flaw, computed only from ES-before/after (independent of game result), matching the thresholds and ladder order in the taxonomy note; unit tests cover the boundary cases (70/30, 85/60) and the deliberate no-impact gap (e.g. 78%→45%).
  3. `game_flaws` has `is_reversed` / `is_squandered` and no `is_while_ahead` / `is_result_changing`, applied by a new forward Alembic migration that upgrades an existing dev DB without a reset; `ty` + the full suite are green.
...
**Success Criteria** (what must be TRUE):
  ...
  8. Mobile + browser-automation parity: `data-testid` / ARIA / semantic HTML on all new interactive elements (card, view-game button, modal); the grid collapses to 1-up on mobile.

**Backend prerequisites:**

  - Add `white_rating` / `black_rating` to the `GET /api/library/flaws` response + `FlawListItem` schema (select/join addition — no new data).
  - New `GET /api/library/games/{game_id}` → a single `GameFlawCard`, scoping the existing list card-builder to one id (for the modal).

**Frontend work:** new `FlawCard` (replaces `FlawRow`) + responsive 2-up grid (replaces the `flex-col gap-3` list); reuse `Card`/`CardHeader`, `LazyMiniBoard`, `SeverityBadge`, `TagChip`/`TagLegend`, `formatCandidateMove`; new `useLibraryGame(id)` hook + a `Dialog` wrapping `LibraryGameCard`.

**Design decisions** (locked in `/gsd-explore` 2026-06-09):

  - **D-01 — 2-up grid + compressed internals** over a wide internal-3-column full-width row: the user prioritized the multi-up page grid; each card is board-left + content stacked on the right.
  - **D-02 — "View game" opens a self-contained modal** on the Flaws tab (not a deep-link/scroll into the Games subtab), avoiding the per-game anchor + offset-paging mechanism the Games list doesn't have.
