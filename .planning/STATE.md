...
---
Last activity: 2026-04-22 — Completed quick task 260422-tnb: endgame insights prompt+data pipeline overhaul (A1-C3). Fixed metric/bucket fan-out (parity_score_pct no longer emitted for conversion bucket), NaN leak into user prompt, fabricated model_used (now set server-side), degenerate n<3 series points filtered, time_pressure_vs_performance dropped from LLM payload. System prompt v2 with auto-generated zone-threshold appendix, subsection→section_id mapping, TC-weighting caveat, tighter trend guidance. _PROMPT_VERSION bumped to endgame_v2 (intentional tier-1 cache invalidation per CONTEXT.md D-08). 88 insights tests + 1024 full suite pass.
