# Requirements: FlawChess — Milestone v2.7 Bot Personas & Playstyle Layer (ARCHIVED)

**Defined:** 2026-07-21
**Closed:** 2026-07-24 (`override_closeout`)
...
- [x] **CAL-04**: Every persona's labeled ELO is a calibrated ELO measured on the Phase-173 internal anchor scale via the Phase-180 harness (~24 cells × ~4 anchors × ~24 games ≈ 2 overnight runs), with a per-persona offset absorbing the style-induced strength delta — **Complete.** Operator sweep produced `reports/data/persona-calibration-cells.tsv` (24 cells); `PERSONA_CALIBRATION_MEASURED=true`; CAL-04b gap-fill `813acd0a` closed the ~800/~1200 rungs. Shipped via PR #274.
- [x] **CAL-05**: Strength labels honor the honesty constraints — bottom rung acknowledges the ~900 measured floor (both weakest Human cells are `beyond_ladder` extrapolations), top rung capped at 1800 (Deep's measured ceiling) — **Complete.** D-07 ceiling clamp, D-06 floor popover, D-04 within-style monotonicity live and covered by `personaRegistry.test.ts`.

### Avatars & Identity (AVAT)

- [ ] **AVAT-01**: 24 AI-generated avatar portraits in one consistent style, manually curated, committed as static assets — **Deferred (accepted at close).** Per locked decision D-16, placeholder emoji-on-tint avatars + committed generation prompts (`260722-r7t`) shipped instead of final portraits. Tracked as a fast-follow, not a milestone gap.
...
- **12 of 13 complete.** PERS-01..04, STYLE-01..05, CAL-04, CAL-05, AVAT-02 all satisfied and verified.
- **1 deferred:** AVAT-01 (real AI portraits) — placeholders + prompts shipped per locked D-16; deferred with the operator's acknowledgment at close.
- CAL-04/CAL-05, originally `[ ]` when the milestone description said "provisional labels," were completed after the fact via the operator overnight sweep + CAL-04b gap-fill (`PERSONA_CALIBRATION_MEASURED=true`).

## Future Requirements

- **SEED-114** (dormant): Ladder extension above ~1900 internal (stronger anchors + raised search budget) to unlock personas above the 1800 rung.
...
| CAL-04 | Phase 184 | Complete |
| CAL-05 | Phase 184 | Complete |
| AVAT-01 | Phase 183 | Deferred (D-16 placeholders shipped) |
| AVAT-02 | Phase 183 | Complete |

**Coverage:** 13 requirements, 13 mapped, 0 unmapped. 12 complete, 1 deferred (AVAT-01).

Note: Phase 185 (Bots roster transpose + win stars) was added mid-milestone from a
2026-07-22 /gsd-explore session; it carried no formal requirement IDs (roster grid
re-layout + server-tracked per-persona win stars) and is folded into v2.7 scope.
