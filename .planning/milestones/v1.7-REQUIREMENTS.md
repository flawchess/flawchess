# Requirements Archive: v1.7 Consolidation, Tooling & Refactoring
...
- [~] **TOOL-04**: Add test coverage analysis and reporting — DROPPED per user decision (not worth the maintenance burden)
...
- [x] **BOPT-01**: Identify and refactor inefficient DB queries (replace row-level processing with aggregations) — SQL COUNT().filter() replaces Python-side loops
- [x] **BOPT-02**: Optimize game_positions column types — verified already optimal, no migration needed (closed)
...
| BOPT-01 | Phase 42 | Complete |
| BOPT-02 | Phase 42 | Complete (verified optimal) |
