...
- **D-19:** Drop `recency` from `TimeSeriesRequest` (`frontend/src/types/position_bookmarks.ts:52`), the matching field in the time-series request schema in `app/schemas/openings.py` (~line 181 — only the time-series one of the three `recency` declarations in that file), and any time-series service / repository call site. Verify no UI surface exposes recency on the bookmark time-series chart — it's been an unused param. Land this first so the date-range refactor doesn't accidentally extend `from`/`to` into a request shape that shouldn't filter at all.
  ...
  **D-19 Amendment (2026-06-13, quick task 260613-bst):** The original D-19 intent ("date-filter-free time-series") has been superseded for totals + emitted points. `TimeSeriesRequest` now accepts optional `from_date`/`to_date` fields (resolved date bounds, not presets). When set, only TimeSeriesPoints whose `played_at` is inside the window are emitted, and WDL totals (`total_wins`, `total_draws`, `total_losses`, `total_games`, `last_played_at`) count only in-window games. The rolling average is still warmed from pre-window games — the repo continues to load the full chronological history so trailing averages anchor correctly. Root cause for the amendment: the bookmark card's WDL bar, game count, and Score % were frozen at full-history values while the Games tab respected the recency filter. The recency PRESET field stays removed (D-19 original intent preserved for the preset shape); only resolved date bounds are added.
