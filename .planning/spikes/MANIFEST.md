...
| 005 | agpl-client-bundle-gate | standard | Given Maia-3 is AGPL-3.0 and the frontend MIT, when Maia ships client-side, then determine whether it forces the frontend to AGPL vs the clean server-side posture | PENDING | maia, license, agpl, seed-081 |
| 006 | moves-by-rating-chart | standard | Given per-ELO move-prob data, when rendered with Recharts, then it matches the reference: per-move lines over ELO, you-are-here marker, top-N ∪ {blunder,best}, theme colors | PENDING | maia, recharts, ui, seed-081 |
