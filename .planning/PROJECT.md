...
---
*Last updated: 2026-05-31 — v1.21 Time-Control-Aware Endgame Metrics in progress; Phase 99 (Percentile Badges for Conversion, Parity, and Recovery) complete. Mirrored the 94.3 per-TC chip pattern onto the Phase 97 Conversion/Parity/Recovery rate cards: 3 new TC-parameterised pooled-per-user rate builders in canonical_slice_sql.py (floor `MINIMUM_RATE_BUCKET_SPANS=30`, source-parity SQL, user_id projected), 12 new `benchmark_metric` ENUM values + 3 bare family names via two additive Alembic migrations (`3981239fd391`, `52c928794fe7` — the latter a Rule-1 fix after the upsert path rejected bare family names and the swallowed error silently wrote 0 rows), COHORT_PERCENTILE_CDF regenerated against the benchmark DB (snapshot 2026-05-30; `parity_rate|classical` empty by D-05 sparsity), and a second title-line PercentileChip per metric block coexisting with the ΔES-gap chip (D-01). Dev backfilled (27 conv / 27 parity / 26 recovery rows); prod backfill deferred to deploy per D-11 sign-off (todo `2026-05-31-phase-99-prod-backfill-rate-percentiles`). Verification 5/5; 2 visual UAT items persist in `99-HUMAN-UAT.md`. Next phase: 99.1 (move cohort CDF out of source into a DB table). NOTE: the "Current Milestone" header above still reads v1.20 LLM Statistical Reasoning and predates v1.21 — drift to reconcile at the next milestone boundary. (Prior footer: 2026-05-24, Phase 94.3 complete — migration `fd5b551f381c`, GLOBAL_PERCENTILE_CDF 4→16, 16 PercentileChip flavors with DIRECTION_BY_FLAVOR.)*
