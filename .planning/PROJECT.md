...
v1.24 Library Page — Phase 104 (single phase, the whole milestone) complete and verified 2026-06-05 (2 plans, local branch `gsd/phase-104-...`). A new top-level **Library** page now hosts the existing Import and Overview workflows as deep-linkable URL-routed `<Tabs variant="brand">` subtabs (`/library/import`, `/library/overview`), mirroring the Openings/Endgames shell pattern; both subtab pages are thin wrappers around the unchanged `ImportPage` / `GlobalStatsPage` (D-01). Top-level nav dropped to **Library · Openings · Endgames (+ Admin)** across desktop nav, mobile bottom bar, and mobile More drawer; the `totalGames === 0` notification dot moved to the Library item on all three surfaces; `/import`, `/overview`, `/rating`, `/global-stats` redirect into the matching subtab; Home's gameless redirect and `ImportRequiredRoute` now target `/library/import`. State-dependent landing (zero games → Import, has games → Overview); Library + both subtabs always reachable. Pure frontend restructure + route migration — no backend or schema changes. LIB-01..09 all satisfied; the one verification gap (a phase-introduced triple-nested `<main>` on `/library/*`) was fixed inline by making the LibraryPage shell a `<div>` (commit f1ff037b). The rest of SEED-036 (Games subtab, Analysis viewer, mistake-detection backend, best-move endpoint) stays unroadmapped in `.planning/seeds/SEED-036-library-page-milestone.md`. Next: `/gsd-complete-milestone` to ship v1.24.
