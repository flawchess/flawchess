...
- [ ] **OPENSTAT-01**: Openings → Stats subtab tables (bookmarked openings + most-played openings) gain an "Avg eval at middlegame entry ± std" column, oriented from the user's POV (positive = user better, regardless of color)
- [ ] **OPENSTAT-02**: Eval significance via one-sample t-test of mean eval vs 0, surfaced with low/medium/high confidence buckets analogous to opening insights cards. T-test confidence reuses the 10-game minimum threshold from opening insights
- [ ] **OPENSTAT-03**: "Avg clock diff at middlegame entry" column on the same tables, analogous to existing "Avg clock diff" column in *Time Pressure at Endgame Entry*
- [ ] **OPENSTAT-04**: Both tables (bookmarked + most-played), both desktop and mobile layouts updated
...
## Current Milestone: v1.16 Stockfish Eval Analyses

**Goal:** Build downstream analyses that consume the Phase 79 middlegame-entry Stockfish evals (and the existing endgame span-entry evals from Phase 78). First phase (80): extend the Openings → Stats subtab tables (bookmarked + most-played) with "Avg eval at middlegame entry ± std", t-test-based eval significance with low/medium/high confidence buckets, and "Avg clock diff at middlegame entry". Together they answer "does this opening leave me better off in position and on the clock when the real fight starts?" Additional phases TBD as analyses are scoped from `.planning/notes/phase-aware-analytics-ideas.md`.

**Target features (Phase 80):**
- Avg eval at middlegame entry ± std, signed user-perspective via existing color-flip helper
- Eval significance via one-sample t-test of mean eval vs 0, surfaced with low/medium/high confidence buckets analogous to opening insights cards
- Avg clock diff at middlegame entry, mirroring SQL pattern from "Avg clock diff at endgame entry" in `app/repositories/endgame_repository.py`
- Both tables (bookmarked + most-played) updated, both desktop and mobile layouts

**Source:** v1.15 Phase 79 populated `phase` SmallInteger and Stockfish eval at the middlegame-entry row across benchmark + prod, opening up phase-aware analyses. Brainstorm in `.planning/notes/phase-aware-analytics-ideas.md` (Active focus section).

## Current State

v1.16 in progress: Phase 80 (opening stats columns), Phase 80.1 (transpositions in Move Explorer), and Phase 81 (Endgame Start vs End twin-tile section) complete on the in-progress milestone. v1.15 Eval-Based Endgame Classification shipped 2026-05-03 (PR #78 combined Phase 78 + 79; PR #79 follow-on `EnginePool` parallelisation). Fifteen milestones complete (v1.0–v1.15), 75 phases (+4 inserted), live at flawchess.com.

Phase 81 added the "Endgame Start vs End" twin-tile section above the existing Endgame Overall Performance WDL table. Tile 1 ("Where you start") shows avg Stockfish eval at endgame entry sig-tested against 0 via Wald-z; Tile 2 ("What you do with it") shows absolute endgame score sig-tested against 50% via Wilson score test. Three-state color (sig positive → green, sig negative → red, not sig → neutral) at p<0.05 with `n ≥ 10` reliability gate. Mobile stacks tiles entry-eval-first per chronological setup → execution; per-tile chart drops below the label-row for full-width readability. Backend `EndgamePerformanceResponse` gains six fields (`entry_eval_mean_pawns`, `entry_eval_n`, `entry_eval_p_value`, `entry_eval_ci_low_pawns`, `entry_eval_ci_high_pawns`, `endgame_score_p_value`). The aggregation runs against `query_endgame_bucket_rows` (one row per game, eval at chronologically first endgame position) so `entry_eval_n + mate_excluded + null_excluded == endgame_wdl.total` by construction — UAT against user 28 surfaced and corrected an earlier per-class pipeline that under-counted by ~5 games per typical user. WDL table, Score Gap column, and "Endgame vs Non-Endgame Score over Time" chart unchanged.
...
---
*Last updated: 2026-05-09 — Phase 81 (Endgame Start vs End twin-tile section) complete. UAT against user 28 surfaced D-22 amendment switching entry-eval source from per-class entry_rows to game-level bucket_rows, eliminating a ~5-game under-count and locking the n + mate + null + total invariant by construction.*

*Previous: 2026-05-03 — v1.15 Eval-Based Endgame Classification shipped (PRs #78, #79). Hard cutover for endgame conv/recov classification; per-position `phase` SmallInteger column populated; middlegame-entry Stockfish eval substrate in place. v1.16 Stockfish Eval Analyses opened with Phase 80 (opening-stats columns for middlegame-entry eval and clock diff). SEED-010 Library milestone now eligible to open.*
