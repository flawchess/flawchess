...
- **AI crawlers are allowed on purpose.** ChatGPT referrals (`utm_source=chatgpt.com` plus the chatgpt.com referrer) are a real acquisition channel, so nothing may disallow AI crawlers. The committed `frontend/public/robots.txt` allows everything except `/import`, `/openings`, `/global-stats`, `/api/`. If the served `https://flawchess.com/robots.txt` ever shows a `# BEGIN Cloudflare Managed content` block (per-bot `Disallow: /` lines for GPTBot, ClaudeBot, Google-Extended, CCBot, ... and a `Content-Signal: ... ai-train=no` line), that block is injected at the Cloudflare edge, not by the repo. Turn it off in the Cloudflare dashboard (there is no API token configured for this zone, dashboard only): zone `flawchess.com` → **AI Crawl Control** → **Settings** → disable **Enable Bot Preference Sync** (Cloudflare's name since Aug 2026 for the feature that prepends the generated block; this removes both the per-bot Disallow lines and the Content Signals Policy line). Then under **AI Crawl Control → Crawlers** set the per-category policy (Search / Agent / Training) to Allow: the injected block is generated from that policy, and a Block there can still stop verified crawlers at the edge with no robots.txt change. Verify with `curl -s https://flawchess.com/robots.txt` (the block is gone once the file starts with `User-agent: *` from the repo file; Cloudflare caches robots.txt, so purge it if the old copy lingers).
