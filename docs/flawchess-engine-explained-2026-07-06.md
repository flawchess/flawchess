...
One thing the user *can* now shift is how that distribution is shaped before truncation. A **"Play style" slider** (§6) lets you tell the engine to model yourself as more human (flatter distribution, more mass on the weaker moves you'd sometimes play) or more precise (sharper, closer to always finding the top move). It reshapes *your own* Maia distribution — the raw list of moves is still Maia's, just re-weighted — and it only affects the modeling, never the "Moves by Rating" chart you see, which always shows Maia's untouched numbers.
...
**The play-style dial — policy temperature.** The slider from §5 is a single knob on how fallible the engine models *you* as being. Technically it applies a "temperature" to your Maia move distribution: turn it up and the distribution flattens (more probability bleeds onto the weaker, noisier moves a real human sometimes plays); turn it down and it sharpens toward always playing the top move, which converges on Stockfish-like precision. Dead center is exactly today's behavior. Turning it up penalizes *fragile* lines — the ones that only work if you keep finding the one hard follow-up — because it raises the modeled chance you go wrong somewhere along them, which drags their averages down and lets simpler, more forgiving moves overtake them. It applies to **your side only**; the opponent stays modeled at their true rating, because flattening *them* would do the opposite (a fallible opponent misses refutations, making your sharp moves look *better*), and a both-sides dial would partly cancel itself. The temperature-adjusted distribution feeds both the search and the findability weighting above, so the two compose instead of fighting.
...
- **It ignores the clock — for now.** Modeling time pressure (players get both weaker *and* noisier when low on time) is not yet automatic. The *mechanism* it would use now exists, though: the "Play style" temperature dial (§6) is exactly the "flatter, noisier distribution" a low clock should produce — today it is a manual slider, not something the engine sets from the clock itself.
