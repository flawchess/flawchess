...
When the budget runs out, the engine reports, for each candidate move on the board: its **practical score**, its **objective Stockfish evaluation**, its **most likely line** (your probable moves plus the opponent's most probable replies, shown as "the line"), and how many times the search visited it. Candidates are ranked by practical score, with exact ties broken alphabetically so the result is perfectly reproducible.
...
- **It ignores the clock — for now.** Modeling time pressure (players get both weaker *and* noisier when low on time) is designed but not yet built.
