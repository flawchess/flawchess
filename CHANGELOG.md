...
- The "Warm-up session" info box on the Train landing page is gone. Guests instead get the sign-up prompt and its "What changes?" / "Sign up free" buttons in the host bot's speech bubble, the same words the guest score screen ends on.
- On the Train landing page, a signed-up account that has not set up training reminders from a phone now gets a one-line nudge in the host bot's speech bubble to install the app and switch reminders on.
- The bot's verdict after a Train puzzle is now one flowing paragraph instead of three stacked lines, so it takes less room above the board on phones.
