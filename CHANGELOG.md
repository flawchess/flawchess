...
- **Phase 87.6: Endgame ELO and Non-Endgame ELO now use FIDE Performance Rating.** The Endgame ELO Timeline switches from the additive K=450 formula to FIDE Performance Rating computed independently for endgame and non-endgame game subsets (`PR = R_opp_avg + 400 * log10(s* / (1 - s*))`). The midpoint of the two ratings approximates the player's actual ELO by construction. Non-Endgame ELO is exposed as a new data point on the timeline, enabling the sentence "Your Endgame ELO sits at X, vs Y in non-endgame games." LLM Insights updated end-to-end; prompt version bumps `endgame_v33` → `endgame_v34` to invalidate cached reports.
