...
class TestPgnToFenPlyHashes:
    ...
    def test_hashes_match_import_pipeline(self) -> None:
        """full_hash from seed must match what process_game_pgn produces during import."""
        from app.services.zobrist import process_game_pgn

        pgn = "1. e4 e5 2. Nf3 Nc6 3. Bb5"
        _fen, _ply, _wh, _bh, seed_full_hash = pgn_to_fen_ply_hashes(pgn)
        # process_game_pgn returns a GameProcessingResult with a "plies" list of
        # PlyData dicts; the last entry is the final position (ply 5).
        result = process_game_pgn(pgn)
        assert result is not None
        import_full_hash = result["plies"][-1]["full_hash"]
