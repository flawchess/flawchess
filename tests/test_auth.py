...
async def register_user(client: httpx.AsyncClient, email: str, password: str) -> dict:
...
class TestRegistration:
    ...
    async def test_register_duplicate_email_returns_400(self):
        ...
        async with httpx.AsyncClient(
            ...
            await register_user(client, "bob@example.com", "password123")
            resp = await register_user(client, "bob@example.com", "password456")
...
class TestLogin:
    ...
    async def test_login_returns_access_token(self):
        ...
        async with httpx.AsyncClient(
            ...
            resp = await login_user(client, "carol@example.com", "password123")
    ...
    async def test_login_wrong_password_returns_400(self):
        ...
        async with httpx.AsyncClient(
            ...
            await register_user(client, "dave@example.com", "password123")
            resp = await login_user(client, "dave@example.com", "wrongpassword")
...
class TestAuthProtection:
    ...
    @pytest.mark.asyncio
    async def test_analysis_with_valid_token_does_not_return_401(self, db_session):
        """POST /analysis/positions with valid Bearer token should not return 401."""
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            await register_user(client, "eve@example.com", "password123")
            login_resp = await login_user(client, "eve@example.com", "password123")
...
class TestUserIsolation:
    ...
    async def test_user_isolation_analysis(self, db_session):
        ...
        from app.repositories import game_repository, position_repository
        ...
        async with httpx.AsyncClient(
            ...
            # Register user A
            resp_a = await register_user(client, "userA@example.com", "password123")
            user_a_id = resp_a.json()["id"]

            # Register user B
            resp_b = await register_user(client, "userB@example.com", "password123")
        ...
        game = Game(
            user_id=user_a_id,
            platform="chess.com",
            platform_game_id="isolation-test-game",
            white_username="userA",
