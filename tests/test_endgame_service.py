...
class TestPerClassPeerDiff:
    """Phase 87 (SEC3-04 / D-01): per-class Conv + Recov peer-diff sig fields.

    Covers the 10 new ConversionRecoveryStats fields wired via two
    compute_score_difference_test calls per class inside _aggregate_endgame_stats:
    - opp_conversion_pct / opp_recovery_pct (0-1 scale parallel to Phase 84's 0-100).
    - opp_conversion_games / opp_recovery_games (mirror sample sizes).
    - conv_diff_p_value / conv_diff_ci_low / conv_diff_ci_high (Conv peer diff).
    - recov_diff_p_value / recov_diff_ci_low / recov_diff_ci_high (Recov peer diff).
    """

    # Map endgame_class string → endgame_class_int (matches EndgameClassInt enum).
    _CLASS_TO_INT = {
        "rook": 1,
        "minor_piece": 2,
        "pawn": 3,
    ...
    @staticmethod
    def _conv_rows(class_int: int, wins: int, draws: int, losses: int) -> list[tuple]:
        """Build conversion-bucket rows (eval >= +100 → white user is on advantage)."""
        rows = []
        for _ in range(wins):
            rows.append((0, class_int, "1-0", "white", 150, None))
        for _ in range(draws):
            rows.append((0, class_int, "1/2-1/2", "white", 150, None))
        for _ in range(losses):
            rows.append((0, class_int, "0-1", "white", 150, None))
        return rows

    @staticmethod
    def _recov_rows(class_int: int, wins: int, draws: int, losses: int) -> list[tuple]:
        """Build recovery-bucket rows (eval <= -100 → white user is on deficit)."""
        rows = []
        for _ in range(wins):
            rows.append((0, class_int, "1-0", "white", -150, None))
        for _ in range(draws):
    ...
    @classmethod
    def _renumber(cls, rows: list[tuple]) -> list[tuple]:
        """Replace placeholder game_id=0 with monotonically increasing ids."""
        out = []
        for i, r in enumerate(rows, start=1):
            out.append((i, r[1], r[2], r[3], r[4], r[5]))
        return out

    def test_conversion_recovery_stats_carries_per_class_diff_fields(self):
        """All 5 surviving classes populate the 10 new fields when both sides are healthy."""
        rows: list[tuple] = []
        # 30 conv (mixed outcomes) + 30 recov (mixed outcomes) per class.
        for class_int in self._CLASS_TO_INT.values():
            rows.extend(self._conv_rows(class_int, wins=18, draws=4, losses=8))
            rows.extend(self._recov_rows(class_int, wins=6, draws=8, losses=16))
        rows = self._renumber(rows)

        result = _aggregate_endgame_stats(rows)
        # All 5 classes should be present (each has 60 games > 0).
        assert len(result) == 5
        for cat in result:
            cs = cat.conversion
            assert cs.opp_conversion_pct is not None
            assert 0.0 <= cs.opp_conversion_pct <= 1.0
            assert cs.opp_recovery_pct is not None
            assert 0.0 <= cs.opp_recovery_pct <= 1.0
            # Mirror identity: opp Conv sample size == user's recovery bucket size.
            assert cs.opp_conversion_games == cs.recovery_games
            assert cs.opp_recovery_games == cs.conversion_games
            # Conv diff sig fields populated (both sides have >= CONFIDENCE_MIN_N games).
            assert cs.conv_diff_p_value is not None
            assert 0.0 <= cs.conv_diff_p_value <= 1.0
            assert cs.conv_diff_ci_low is not None
            assert cs.conv_diff_ci_high is not None
            assert cs.conv_diff_ci_low <= cs.conv_diff_ci_high
            # Recov diff sig fields populated symmetrically.
            assert cs.recov_diff_p_value is not None
            assert 0.0 <= cs.recov_diff_p_value <= 1.0
            assert cs.recov_diff_ci_low is not None
            assert cs.recov_diff_ci_high is not None
            assert cs.recov_diff_ci_low <= cs.recov_diff_ci_high

    def test_per_class_conv_diff_mirror_flip_correctness(self):
        """Mirror-flip math: user Conv 90% / user Recov 50% → opp Conv 50%, opp Recov 10%.

        User rook conv W=18/D=0/L=2 (chess-score=0.9), recov W=10/D=0/L=10 (chess-score=0.5).
        Opp Conv = mirror-flip W↔L on recov bucket: (W=10, D=0, L=10) → chess-score=0.5.
        Conv diff = 0.9 - 0.5 = 0.4 (large positive).
        Opp Recov = saves-as-W on conv bucket: ((L+D)=2, 0, W=18) → chess-score=0.1.
        Recov diff (saves-as-W mapping): user (W+D=10, 0, L=10)=0.5 vs opp 0.1 = 0.4 positive.
        """
        rows = self._conv_rows(1, wins=18, draws=0, losses=2) + self._recov_rows(
            1, wins=10, draws=0, losses=10
        )
        rows = self._renumber(rows)
        ...
        # Mirror-rate identities on the 0-1-scale opp_* fields.
        assert cs.opp_conversion_pct == pytest.approx(0.5, abs=0.001)
        assert cs.opp_recovery_pct == pytest.approx(0.1, abs=0.001)

        # Conv diff midpoint and CI.
        conv_mid = 0.5 * (cast_or_zero(cs.conv_diff_ci_low) + cast_or_zero(cs.conv_diff_ci_high))
        assert conv_mid == pytest.approx(0.4, abs=0.01)
        assert cs.conv_diff_ci_low is not None and cs.conv_diff_ci_low > 0  # CI excludes 0
        assert cs.conv_diff_ci_high is not None and cs.conv_diff_ci_high > 0
        # Highly significant: diff=0.4, n=20 per side (Wald-z; empirical variance).
        assert cs.conv_diff_p_value is not None
        assert cs.conv_diff_p_value < 0.01

        # Recov diff midpoint: user save-rate 0.5 - opp save-rate 0.1 = 0.4 positive.
        recov_mid = 0.5 * (cast_or_zero(cs.recov_diff_ci_low) + cast_or_zero(cs.recov_diff_ci_high))
        assert recov_mid == pytest.approx(0.4, abs=0.01)
        assert cs.recov_diff_ci_low is not None and cs.recov_diff_ci_low > 0
        assert cs.recov_diff_p_value is not None
        assert cs.recov_diff_p_value < 0.01

    def test_per_class_conv_diff_sparse_opp_gated_to_none(self):
        """Asymmetric gating: p_value requires min(n) >= 10; CI requires min(n) >= 2."""
        # conversion_games=50, recovery_games=5: 5 < 10 (p-gate) but 5 >= 2 (CI-gate).
        rows = self._conv_rows(1, wins=30, draws=0, losses=20) + self._recov_rows(
            1, wins=2, draws=0, losses=3
        )
        rows = self._renumber(rows)
        ...
        rook = next(c for c in result if c.endgame_class == "rook")
        cs = rook.conversion
        assert cs.conversion_games == 50
        assert cs.recovery_games == 5
        # p_value gated (min(50, 5) = 5 < CONFIDENCE_MIN_N=10).
        assert cs.conv_diff_p_value is None
        assert cs.recov_diff_p_value is None
        # CI still computed (min n=5 >= 2).
        assert cs.conv_diff_ci_low is not None
        assert cs.conv_diff_ci_high is not None
        assert cs.recov_diff_ci_low is not None
        assert cs.recov_diff_ci_high is not None

        # Now go below the CI gate too: recovery_games=1.
        rows2 = self._conv_rows(1, wins=30, draws=0, losses=20) + self._recov_rows(
            1, wins=0, draws=0, losses=1
        )
        rows2 = self._renumber(rows2)
        result2 = _aggregate_endgame_stats(rows2)
        rook2 = next(c for c in result2 if c.endgame_class == "rook")
        cs2 = rook2.conversion
        assert cs2.recovery_games == 1
        assert cs2.conv_diff_p_value is None
        assert cs2.conv_diff_ci_low is None
        assert cs2.conv_diff_ci_high is None
        assert cs2.recov_diff_p_value is None
        assert cs2.recov_diff_ci_low is None
        assert cs2.recov_diff_ci_high is None

    def test_per_class_diff_zero_games_safety(self):
        """Class with conversion_games=0 AND recovery_games=0: all diff fields None, no crash."""
        # Parity-only rows for queen (class_int=4): eval in (-100, +100).
        rows = [
            (1, 4, "1-0", "white", 0, None),
            (2, 4, "0-1", "white", 50, None),
            (3, 4, "1/2-1/2", "white", -50, None),
        ]
        result = _aggregate_endgame_stats(rows)
        queen = next(c for c in result if c.endgame_class == "queen")
        cs = queen.conversion
        assert cs.conversion_games == 0
        assert cs.recovery_games == 0
        # All sig fields None (helper returns None triple when either n <= 0).
        assert cs.conv_diff_p_value is None
        assert cs.conv_diff_ci_low is None
        assert cs.conv_diff_ci_high is None
        assert cs.recov_diff_p_value is None
        assert cs.recov_diff_ci_low is None
        assert cs.recov_diff_ci_high is None
        # 0-1-scale opp_* mirror-rate fields None (sample size 0 < _MIN_OPPONENT_SAMPLE).
        assert cs.opp_conversion_pct is None
        assert cs.opp_recovery_pct is None
        assert cs.opp_conversion_games == 0
        assert cs.opp_recovery_games == 0


def cast_or_zero(value: float | None) -> float:
    """Helper for midpoint calc in mirror-flip test: treat None as 0.0 (only used when both bounds are non-None)."""
