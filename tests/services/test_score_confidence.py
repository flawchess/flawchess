...
def test_medium_at_moderate_n() -> None:
    confidence, _p_value = compute_confidence_bucket(w=6, d=6, losses=18, n=30)
    assert confidence == "medium"


def test_low_at_n10_extreme_score() -> None:
    confidence, _p_value = compute_confidence_bucket(w=2, d=2, losses=6, n=10)
    assert confidence == "low"


def test_just_inside_medium_boundary() -> None:
    # half_width is just inside 0.20; integer (w, d, l, n) cannot hit 0.20 exactly.
    confidence, _p_value = compute_confidence_bucket(w=5, d=5, losses=15, n=25)
    assert confidence == "medium"


def test_p_value_at_score_050_is_one() -> None:
    _confidence, p_value = compute_confidence_bucket(w=8, d=4, losses=8, n=20)
