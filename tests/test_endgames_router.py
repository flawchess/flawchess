...
class TestOverviewScoreGapMaterial:
    ...
    async def test_overview_has_score_gap_material_field(
        ...
        # Phase 87.2 (D-06): per-bucket score_gap_* fields on the response.
        # Phase 87.4 (D-05): score_gap_skill_* dropped alongside Skill retirement.
        assert "score_gap_conv_mean" in sgm
        assert "score_gap_parity_mean" in sgm
        assert "score_gap_recov_mean" in sgm
        assert "score_gap_skill_mean" not in sgm
