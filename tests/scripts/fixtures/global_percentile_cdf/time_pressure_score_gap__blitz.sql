...
per_user AS (
  SELECT
    user_id,
    avg(user_score)
      FILTER (WHERE user_clock_pct < 0.40) AS user_pressured_score,
    avg(1.0 - user_score)
      FILTER (WHERE opp_clock_pct < 0.40) AS opp_pressured_score,
    count(*)
      FILTER (WHERE user_clock_pct < 0.40) AS user_pressured_n,
    count(*)
      FILTER (WHERE opp_clock_pct < 0.40) AS opp_pressured_n
  FROM joined
