...
async def _make_game_with_phase_entries(
    ...
    *,
    ...
    """Create one Game + GamePosition rows (opening anchor + MG-entry + opp-clock row).
...
class TestOpeningPhaseEntryMetrics:
    ...
    @pytest.mark.asyncio
    async def test_mate_excluded_from_eval(self, db_session: AsyncSession) -> None:
    ...
    async def test_recency_filter_threading(self, db_session: AsyncSession) -> None:
        """Recency filter excludes pre-cutoff games from eval_n_mg."""
        full_hash = 10_014
        ...
        await _make_game_with_phase_entries(
            db_session, full_hash=full_hash, mg_eval_cp=80, played_at=recent_date
        )
