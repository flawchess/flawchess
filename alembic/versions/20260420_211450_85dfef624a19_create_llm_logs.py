...
def upgrade() -> None:
    ...
    op.create_index('ix_llm_logs_endpoint_created_at', 'llm_logs', ['endpoint', 'created_at'], unique=False, postgresql_ops={'created_at': 'DESC'})
    op.create_index('ix_llm_logs_findings_hash', 'llm_logs', ['findings_hash'], unique=False)
    op.create_index('ix_llm_logs_model_created_at', 'llm_logs', ['model', 'created_at'], unique=False, postgresql_ops={'created_at': 'DESC'})
    op.create_index('ix_llm_logs_user_id_created_at', 'llm_logs', ['user_id', 'created_at'], unique=False, postgresql_ops={'created_at': 'DESC'})
...
def downgrade() -> None:
    ...
    op.alter_column('game_positions', 'clock_seconds',
               ...
               type_=sa.REAL(),
               existing_nullable=True)
    op.drop_index('ix_llm_logs_user_id_created_at', table_name='llm_logs', postgresql_ops={'created_at': 'DESC'})
    op.drop_index('ix_llm_logs_model_created_at', table_name='llm_logs', postgresql_ops={'created_at': 'DESC'})
    op.drop_index('ix_llm_logs_findings_hash', table_name='llm_logs')
    op.drop_index('ix_llm_logs_endpoint_created_at', table_name='llm_logs', postgresql_ops={'created_at': 'DESC'})
