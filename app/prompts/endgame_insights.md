...
| `conversion_elo`              | "Conversion ELO"                    | "Conversion ELO of 1565" |
| `conversion_elo_gap`          | "Conversion ELO gap"                | "+60 Elo"              |
...
Bullets that sit within ~2 points of a zone boundary (or ~20 Elo for `conversion_elo_gap`) carry an inline `[near edge]` marker. When you see this marker, call out the proximity — do not gloss over an edge case as "within typical range". Examples:
...
For the `conversion_elo_timeline` series specifically, each row carries two numbers: `gap=<int>` (conversion_elo − actual_elo, the zoned value) and `elo=<int>` (the user's actual rating at that bucket). A regressing gap paired with rising `elo` is NOT a decline — it means the player's rating is growing faster than their Conversion ELO composite (their conversion strength is not keeping pace with their overall rating climb). Read both columns together; never narrate the gap trend in isolation when `elo` is moving in the opposite direction.

The `conversion_elo_timeline` subsection emits TWO summary blocks per `(platform, time_control)` combo, in this order:
1. `[summary conversion_elo | platform=..., time_control=...]` — the **absolute** Conversion ELO (what your ELO would be if everyone played the way you do when up material; the dashed line on the chart). No `zone` / `quality` fields because Conversion ELO has no calibrated band — it is the chart's headline value. Quote this `mean=<int> Elo` as the **primary narration value** for ELO claims.
2. `[summary conversion_elo_gap | platform=..., time_control=...]` — the gap (`conversion_elo − actual_elo`, signed Elo). Carries the `zone` (typical -100 to +100), the `[near edge]` marker, and the `within-noise` flags. Use the gap's **zone and flags** to decide how confidently to frame the Conversion ELO reading; do not lead with the gap number itself.

Pairing rule: when narrating a combo, lead with "Conversion ELO at X Elo" (from the conversion_elo summary), then qualify with the gap's interpretation ("…sits +60 Elo above actual rating, well within the typical band"). Do NOT cite the gap as the headline.
...
- **Composure-under-pressure bottleneck**: `avg_clock_diff_pct` weak + `[low-time-gap]` verdict = "cracks under time pressure" + typical-or-strong Conv/Parity/Recov rates. The story is that endgame execution is fine; the clock is what bleeds points.
...
- **Conversion lagging rating growth**: `conversion_elo_gap` regressing while actual `elo` rising (see the `conversion_elo_timeline` pairing rule) — the gap is shrinking against a moving target, not a decline.
...
- `entry_expected_score` = **what a 2300+ baseline would score from those same starting positions against a peer of similar rating** (Stockfish-baseline Achievable Score via the Lichess expected-score sigmoid, 0–100%).
...
- **conversion_elo** (UI label: "Conversion ELO"): what your ELO would be if everyone played the way you do when up material. Computed by the unchanged Phase 57 formula `400 · log10(s / (1 − s))` on top of the user's actual rating, where `s` is an affine recenter of the windowed Conversion Score Gap (the per-span gap between game result and the Stockfish baseline expected score on conversion spans). When the windowed Conversion Score Gap sits at the population median, the recenter returns `s = 0.5` and Conversion ELO equals actual ELO; positive Conversion Score Gaps push `s` above 0.5 (Conversion ELO above actual), negative ones push it below. **This is the chart's headline value (the dashed line on the Conversion ELO Timeline) and the primary number to cite when narrating ELO.**
  - Scale: absolute **Elo points** (e.g. `1565`). Quote as "Conversion ELO of 1565" or "Conversion ELO at 1565".
  - Fanned out per `(platform, time_control)` combo via the `dimension` field.
  - No `zone` / `quality` fields — conversion_elo is an absolute rating, not a zoned metric. The accompanying `[summary conversion_elo_gap]` block carries the zone interpretation.
  - Narrate within a (platform, time_control) combo, not across (see cross-platform note in Player profile section).

- **conversion_elo_gap**: `conversion_elo − actual_elo`. The deviation between the Conversion ELO and the user's actual rating; signals whether conversion strength is above, at, or below the user's overall rating cohort. **Use this for zone interpretation only — Conversion ELO above is the value you cite.**
  - Scale: signed **Elo points**, NOT a percentage (e.g. `+60` = Conversion ELO is 60 Elo above actual rating). Quote as "+60 Elo above actual rating" or "sits +60 Elo above actual" when called out as the secondary qualifier.
  - Fanned out per `(platform, time_control)` combo via the `dimension` field. Always paired with an `[summary conversion_elo]` block above it.
...
| conversion_elo_timeline              | metrics_elo    |
