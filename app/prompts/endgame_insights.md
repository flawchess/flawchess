...
For the `endgame_elo_timeline` series specifically, each row carries three numbers: `gap=<int>` (endgame_elo − actual_elo, the zoned value), `elo=<int>` (the user's actual rating at that bucket), and `non_eg_elo=<int>` (the FIDE Performance Rating computed from the non-endgame game subset). A regressing gap paired with rising `elo` is NOT a decline — it means the player's actual rating is growing faster than their endgame is *lifting* it (the offset is shrinking against a moving target). Read all three columns together; the midpoint property holds: `midpoint(elo, non_eg_elo) ≈ actual rating` by construction, so divergence between `elo` and `non_eg_elo` reveals which side of the player's game is driving rating.

The `endgame_elo_timeline` subsection emits THREE summary blocks per `(platform, time_control)` combo, in this order:
1. `[summary endgame_elo | platform=..., time_control=...]` — the **absolute** Endgame ELO (FIDE Performance Rating from endgame game subset). No `zone` / `quality` fields because Endgame ELO has no calibrated band — it is the chart's headline value. Quote this `mean=<int> Elo` as the **primary narration value** for ELO claims.
2. `[summary non_endgame_elo | platform=..., time_control=...]` — the **absolute** Non-Endgame ELO (FIDE Performance Rating from non-endgame game subset). No `zone` / `quality` fields. The midpoint property holds: `midpoint(endgame_elo, non_endgame_elo) ≈ actual_elo` by construction. Quote this alongside Endgame ELO to show which side drives the player's rating.
...
- **endgame_elo** (UI label: "Endgame ELO"): FIDE Performance Rating computed independently from your endgame game subset. Formula: `PR = R_opp_avg + 400 * log10(s* / (1 - s*))` with Laplace-smoothed score `s* = (n * score + 1) / (n + 2)`, where `score` is your result average (win=1, draw=0.5, loss=0) against opponents whose average rating is `R_opp_avg`. The midpoint property holds: `midpoint(endgame_elo, non_endgame_elo) ≈ actual_elo` by construction. When Endgame ELO exceeds Non-Endgame ELO, your endgame play is *lifting* your rating; when it falls below, your endgame is *holding back* your rating. **This is the chart's headline value and the primary number to cite when narrating ELO.**
...
- **non_endgame_elo** (UI label: "Non-Endgame ELO"): FIDE Performance Rating computed independently from your non-endgame game subset (games that did NOT reach an Endgame Phase). Same formula as `endgame_elo` but applied to the complementary game subset. The midpoint of `endgame_elo` and `non_endgame_elo` approximates your actual rating by construction — divergence between the two reveals which side of your game is driving your rating. Quote alongside Endgame ELO using the sentence skeleton: "Your Endgame ELO sits at X, vs Y in non-endgame games."
