...
from app.schemas.endgames import EndgameClass, TimePressureTcCard
...
def _format_zone_bounds(metric_id: str, dimension: dict[str, str] | None) -> str:
    ...
    # Phase 102 UAT: metrics_elo findings carry a `time_control` dimension and
    # their zone band is per-TC (TC_METRIC_BANDS), matching the page's per-TC
    # gauges. type_breakdown findings carry `endgame_class` (not time_control)
    # so the per-class branch below still wins for them.
