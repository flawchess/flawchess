...
def _conversion_elo_per_bucket(points: list[TimePoint] | None) -> list[tuple[float, int]]:
    """Return [(conversion_elo_value, n), ...] derived from conversion_elo_gap series points.

    Conversion ELO per bucket = actual_elo + gap. The gap metric is non-fractional
    (scale=1.0), so `pt.value` is already in Elo units. Points missing
    `actual_elo` are skipped — the gap series carries it for conversion_elo_gap
    only (see _render_series_block).

    Phase 87.4 (D-06): renamed from `_endgame_elo_per_bucket` in lockstep with
    the metric rename `endgame_elo_gap` → `conversion_elo_gap`.
    """
...
def _render_conversion_elo_summary_block(
    *,
    dim_key: str,
    ...
    """Emit `[summary conversion_elo | dim]` derived from the conversion_elo_gap series.

    v11: the chart's headline value is Conversion ELO (absolute Elo, skill-adjusted)
    ...
    without doing arithmetic. No zone / quality fields — conversion_elo has no
    calibrated band; the paired `[summary conversion_elo_gap]` block (rendered
    immediately after this one) carries the zone interpretation.

    Phase 87.4 (D-06): renamed from `_render_endgame_elo_summary_block` and
    every `endgame_elo*` identifier in lockstep with the metric rename.
    """
    ...
    def window_line(
        ...
        values = _conversion_elo_per_bucket(series)
        if not values:
            return None, None
    ...
    # Phase 87.4 (D-06): header literal renamed from "[summary endgame_elo".
    header = "[summary conversion_elo"
    if dim_key:
        header += f" | {dim_key}"
    header += "]"
