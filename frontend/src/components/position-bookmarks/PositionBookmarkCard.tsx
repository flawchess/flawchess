...
export function PositionBookmarkCard({ bookmark, onLoad, chartEnabled, onChartEnabledChange, onMatchSideChange }: Props) {
  ...
  return (
    <div
      ...
      style={style}
      {...attributes}
      className="flex flex-col gap-2 rounded-md bg-card px-3 py-2"
      data-testid={`bookmark-card-${bookmark.id}`}
    >
      {/* Top row: drag handle + color circle + label (full width) */}
      ...
      {/* Body row: mini board left, piece filter + buttons right */}
