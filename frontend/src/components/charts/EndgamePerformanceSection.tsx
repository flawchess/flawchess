  ...
  return (
    <div className="space-y-1" data-testid={testId}>
      ...
      <div className="flex gap-3 text-xs text-muted-foreground">
        <span style={{ color: WDL_WIN }}>W: {wdl.win_pct.toFixed(0)}%</span>
        <span style={{ color: WDL_DRAW }}>D: {wdl.draw_pct.toFixed(0)}%</span>
        <span style={{ color: WDL_LOSS }}>L: {wdl.loss_pct.toFixed(0)}%</span>
      </div>
    </div>
  );
}
...
export function EndgamePerformanceSection({ data }: EndgamePerformanceSectionProps) {
  return (
    <div className="space-y-4">
      <h3 className="text-base font-semibold">Endgame Performance</h3>
