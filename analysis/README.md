...
| `session_length_study/` | Session-length story candidate (GM Studer's "ideal number of games per sitting, per time control"): residual by game number in the session (marginal, user-demeaned, fixed-cohort), games vs minutes, within-user blunder rate and think time by position, warm-up, in-session pauses, fatigue-vs-tilt, stop hazard. Plain script over the tilt study's cached extracts (no DB needed); findings in `session_length_study/FINDINGS.md`. |
